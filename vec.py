...
def add(u, v):
    ...
    for e1 in diccU:
        for e2 in diccV:
            if e1 == e2:
                diccS[e1] = diccU[e1] + diccV[e2]

    return Vec(u.D, diccS)
...
def dot(u, v):
    ...
    for e1 in diccU:
        for e2 in diccV:
            if e1 == e2:
                dotlist.append(diccU[e1] * diccV[e2])
...
def scalar_mul(v, alpha):  # No es conmutativo
    """
...
class Vec:
    ...
    def __str__(v):
        ...
        wd = dict([(k, (1+max(len(str(k)), len('{0:.{1}G}'.format(v[k], numdec))))) if isinstance(
            v[k], int) or isinstance(v[k], float) else (k, (1+max(len(str(k)), len(str(v[k])))))
            for k in D_list])

        s1 = ''.join(['{0:>{1}}'.format(str(k), wd[k]) for k in D_list])

        s2 = ''.join(['{0:>{1}.{2}G}'.format(v[k], wd[k], numdec) if isinstance(
            v[k], int) or isinstance(v[k], float) else '{0:>{1}}'.format(v[k], wd[k])
            for k in D_list])
